...
Lab 09 - FP16 混合精度训练
===========================
使用 torch.cuda.amp 进行 FP16 混合精度训练。

关键:
  - GradScaler 动态调整 loss scale，防止梯度下溢
  - 前向/反向用 FP16，优化器用 FP32
  - autocast 自动选择哪些操作用 FP16
...
def train(args):
    ...
    # ====== FP16 关键: GradScaler ======
    # 初始 scale = 2^16 = 65536
    # 动态调整: 如果没有 overflow → scale *= 2; 如果 overflow → scale /= 2
    ...
    for epoch in range(args.epochs):
        ...
        for step, (input_ids, targets) in enumerate(dataloader):
            ...
            # ====== autocast: 自动将合适的操作转为 FP16 ======
            with autocast(dtype=torch.float16):
                logits = model(input_ids)
                loss = loss_fn(logits.view(-1, 32000), targets.view(-1))

            # ====== scaler: 缩放 loss → 放大梯度 → 防止下溢 ======
            scaler.scale(loss).backward()

            # unscale 梯度（用于 grad clip）
            ...
            if rank == 0 and step % 50 == 0:
                ...
                print(f"  Step {step:4d} | Loss {loss.item():.4f} | "
                      f"Scale {scale:.0f} | Mem {mem:.2f} GB")

        if rank == 0:
            elapsed = time.perf_counter() - t0
            print(f"Epoch {epoch} | Time {elapsed:.1f}s | Final Scale {scaler.get_scale():.0f}")

    if rank == 0:
        print(f"\n=== FP16 训练完成 | 峰值显存: {torch.cuda.max_memory_allocated(device)/1e9:.2f} GB ===")
